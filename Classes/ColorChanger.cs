...
namespace VapeMenu.Classes
{
    public class ColorChanger : TimedBehaviour
    {
        ...
        public override void Update()
        {
            ...
            if (colors != null)
            {
                if (!VapeMenu.Menu.Main.dynamicGradients)
                {
                    if (!isMonkeColors)
                    {
                        if (timeBased)
                ...
                else
                {
                    ...
                    } else
                    {
                        if (gameObjectRenderer.material.shader.name != "Universal Render Pipeline/Lit")
                        {
                            ...
                            gameObjectRenderer.material.mainTexture = VapeMenu.Menu.Main.GetGradientTexture(colors.colorKeys[0].color, colors.colorKeys[1].color);
                        }
                    }
                }
            }
