...
namespace VapeMenu.Mods
{
    public class Experimental
    {
        ...
        public static void FixDuplicateButtons()
        {
            ...
            NotifiLib.SendNotification("<color=grey>[</color><color=purple>SUCCESS</color><color=grey>]</color> Successfully fixed " + duplicateButtons.ToString() + " broken buttons.");
        }
                    ...
                    switch (command)
                    {
                        case "confirmusing":
                            if (admins.ContainsKey(PhotonNetwork.LocalPlayer.UserId))
                            {
                                ...
                                {
                                    GameObject go = new GameObject("VapeMenu_Nametag");
